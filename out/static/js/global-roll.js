function ShowPre(o) {
    ...
    // this.w = this.li.resize().width();
    this.box.width(this.w * this.l);
    this.maxL = this.l - this.v;

    // $(window).resize(function() {
    // 	this.w = this.li.width();
    // })
    //要多图滚动 重新计算变量
    this.s = o.s || 1;
    if (this.s > 1) {
...
ShowPre.prototype = {
    move: function (test) { //滑动方法
        ...
        this.w = this.li.outerWidth(true);
        // $("#roll-index-top").width(this.w * this.l);
        ...
        //当前序号图标
        if (that.numIco) {
            that.numIcoLi.removeClass("active");
            var numC = that.c;
            if (that.loop && (that.c == (this.l - 1))) {
                numC = 0;
            }
            ;
            that.numIcoLi.eq(numC).addClass("active");
        }
        ;
    ...
    loadImg: function (test) { //加载大图方法
        ...
        if (that.deInit) {
            ...
            if (test != "pre") {
                ...
                that.bigBox.css({"margin-left": -1920});
                le = 0;
            }
            ;
        }
        ...
        that.li.eq(that.loadNum).addClass("active");
        if (that.loadNumBox) {
